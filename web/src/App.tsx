  ...
  return (
    <div className="app" ref={appRef}>
      ...
      <div className="toolbar" ref={toolbarRef}>
        ...
        {mode === 'fixed' && (
          <button className="howToBtn" onClick={async () => {
            try {
              // Build a 5x5 blurred mosaic image from filled cells; if a cell is empty, render a neutral block
              const cols = 5, rows = 5;
              const tile = 64; // base tile size; final will scale
              const pad = 2;
              ...
              for (let r = 0; r < rows; r++) {
                for (let c = 0; c < cols; c++) {
                  ...
                  const x = c * (tile + pad);
                  const y = r * (tile + pad);
                  const poster = filled[i]?.filledBy?.posterUrl;
                  if (poster) {
                    try {
                      const proxied = `/api/proxy-image?url=${encodeURIComponent(poster)}`;
                      const img = new Image();
                      ...
                      ctx.save();
                      // Extreme blur by drawing scaled down then back up
                      const off = document.createElement('canvas');
                      const offCtx = off.getContext('2d')!;
                      const s = 6; // downscale factor for heavy blur effect
                      off.width = Math.max(2, Math.floor(tile / s));
                      off.height = Math.max(2, Math.floor(tile / s));
                      offCtx.drawImage(img, 0, 0, off.width, off.height);
                      ctx.imageSmoothingEnabled = true;
                      ctx.imageSmoothingQuality = 'low';
                      ctx.drawImage(off, x, y, tile, tile);
                      // add slight rounded corners mask
                      ctx.globalCompositeOperation = 'destination-in';
                      ctx.beginPath();
                      const rds = 6;
                      ctx.moveTo(x + rds, y);
                      ctx.arcTo(x + tile, y, x + tile, y + tile, rds);
                      ctx.arcTo(x + tile, y + tile, x, y + tile, rds);
                      ctx.arcTo(x, y + tile, x, y, rds);
                      ctx.arcTo(x, y, x + tile, y, rds);
                      ctx.closePath();
                      ctx.fill();
                      ctx.globalCompositeOperation = 'source-over';
                      ctx.restore();
                    } catch {
                      ctx.fillStyle = '#222c3e';
              ...
              // Try share; fallback to open in new tab
              if (typeof (navigator as any).share === 'function') {
                try {
                  const blob = await (await fetch(dataUrl)).blob();
                  const file = new File([blob], 'afgf-fixed-share.png', { type: 'image/png' });
                  await (navigator as any).share({ files: [file], title: 'A Few Good Films - Fixed mode' });
                  return;
                } catch {}
              }
              const a = document.createElement('a');
              a.href = dataUrl;
              a.download = 'afgf-fixed-share.png';
              a.click();
            } catch {}
          }}>Share Mosaic</button>
        )}
